function [FHANDLE] = plot_logs_with_acados(logs, FADED, LOGSCALE, FHANDLE, xlims, ylims)
...
for ii = 1:nexp
    ...
    data(kk).y1 = [data(kk).y1 logs{ii}.cputime - logs{ii}.simtime];
    data(kk).y2 = [data(kk).y2 logs{ii}.acados_qptime];
...
if ~LOGSCALE
    ...
    for kk = 1:length(data)
        plot(data(kk).x, 1e3*(max(data(kk).y1)), ...
            'Marker', data(kk).marker, 'MarkerSize', 12, 'MarkerEdgeColor', black + 1 - alpha, ...
            'Color', [black alpha], 'Linewidth',1.5, 'LineStyle', style);
        hold on
        plot(data(kk).x, 1e3*(max(data(kk).y2)), ...
            'Marker', data(kk).marker, 'MarkerSize', 12, 'MarkerEdgeColor', red + 1 - alpha, ...
            'Color', [red alpha], 'Linewidth',1.5, 'LineStyle', style);
        hold on
...
%     ylim(ylims)
    ...
    for kk = 1:length(data)
        loglog(data(kk).x, 1e3*max(data(kk).y1), ...
            'Marker', data(kk).marker, 'MarkerSize', 12, 'MarkerEdgeColor', black + 1 - alpha, ...
            'Color', [black alpha], 'linewidth',1.5, 'LineStyle', style);
        hold on
        loglog(data(kk).x, 1e3*(max(data(kk).y2)), ...
            'Marker', data(kk).marker, 'MarkerSize', 12, 'MarkerEdgeColor', red + 1 - alpha, ...
            'Color', [red alpha], 'Linewidth',1.5, 'LineStyle', style);
        hold on
    end
